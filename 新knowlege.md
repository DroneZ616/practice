...
- [***十二， 函数***](#十二-函数)
  ...
  - [***4、`lambda` 函数***](#4lambda-函数)
    - [1.基础定义](#1基础定义)
    - [2.语法结构（非常短）](#2语法结构非常短)
    - [3.用途：作为参数](#3用途作为参数)
  - [***5、拆包（Unpacking）***](#5拆包unpacking)
  - [***6、内置函数`map`、`filter`、`zip`***](#6内置函数mapfilterzip)
- [***十三，作用域***](#十三作用域)
  - [作用域的 4 种类型（LEGB 规则）](#作用域的-4-种类型legb-规则)
    - [局部作用域](#局部作用域)
    - [全局作用域](#全局作用域)
    - [函数内部修改全局变量（`global`）](#函数内部修改全局变量global)
    - [`globals()` 和 `locals()`（内置函数）](#globals-和-locals内置函数)
    - [嵌套函数与 `nonlocal`（扩展了解）](#嵌套函数与-nonlocal扩展了解)
- [***十五，模块***](#十五模块)
...
`lambda` 是 Python 里创建**匿名函数**的方式。匿名函数的意思是：这个函数**没有名字**。

- 普通函数用 `def` 定义，并且需要一个名字。
- `lambda` 不需要名字，写出来就是为了“临时用一下”或“传给别的函数”。
---
### 2.语法结构（非常短）
...
```
注意：虽然这样能跑，但既然你给它起了名字，就失去了“匿名”的意义。这样写不如直接用 `def` 更清晰。

---

### 3.用途：作为参数

`lambda` 最常用的时候，是把它 **直接写进另一个函数的括号里**，不取名字，用完就丢。
...
players.sort(key=lambda p: p["age"])
print(players)   # B 在前（18 < 25）
```

- `sort` 方法需要一个 `key` 参数，用来告诉它“按什么排序”。
- `lambda p: p["age"]` 就是这个“规则”：从每个玩家字典里取出 `"age"` 的值，按它排序。
- 这个函数只用这一次，不需要单独定义，直接用 `lambda` 写在这一行就够了。

---
1. 两个限制（必须清楚）

2. **只能写一行表达式**，不能写多行，不能写 `print`，不能写赋值语句。
3. **不适合复杂逻辑**：如果逻辑超过一行，或者需要多个步骤，就该用 `def` 定义普通函数。
...
你已经见过最基本的拆包了：
```python
x, y = 1, 2          # 把元组 (1,2) 拆开赋值给 x 和 y
first, *middle, last = [1, 2, 3, 4, 5]  # * 把中间的都收进 middle 列表
```
但拆包不止于此。* 和 ** 还可以用在函数调用和函数定义中。

1. 在函数调用时拆包（把列表/字典拆成位置参数/关键字参数）
*：把列表或元组拆成位置参数。
...
# 用 * 拆包列表
info_list = ["李四", 30, "上海"]
introduce(*info_list)   # 相当于传入 "李四", 30, "上海"

# 用 ** 拆包字典（注意键名必须和参数名一致）
info_dict = {"name": "王五", "age": 28, "city": "深圳"}
introduce(**info_dict)  # 相当于传入 name="王五", age=28, city="深圳"
```

2. 在函数定义时收集参数（把多个位置参数/关键字参数打包）
- *args：把多余的位置参数打包成元组。
...
collect(1, 2, 3, name="小明", age=18)
# 输出：
# 位置参数: (1, 2, 3)
# 关键字参数: {'name': '小明', 'age': 18}
```
注：`args`和`kwargs`不是关键字，`*`或`**`后其实可接上任何变量名，例如：
...
## ***6、内置函数`map`、`filter`、`zip`***

这三个函数都接收一个函数和一个可迭代对象，对每个元素进行处理，并返回一个新的迭代器（或可迭代对象）。

1. map(function, iterable) —— “批量加工”
   
对可迭代对象中的每个元素都调用一次函数，返回一个由每次函数返回值组成的新迭代器。
...
nums = [1, 2, 3, 4]
result = map(double, nums)   # map 返回一个迭代器
print(list(result))          # [2, 4, 6, 8] （用 list() 展开查看内容）
```
它最常和 lambda 配合使用：
...
```
2. filter(function, iterable) —— “筛选器”

对可迭代对象中的每个元素调用函数，只保留函数返回 True 的那些元素。
...
```
3. zip(*iterables) —— “拉链合并”

把多个可迭代对象中同一位置的元素打包成元组，返回一个由这些元组组成的迭代器。
...
print(list(result))  # [('小明', 90), ('小红', 85), ('小刚', 88)]
```
它非常适合同时遍历多个列表：
...
作用域（Scope）就是变量在代码中可以被访问到的“地盘”。Python 查找变量时有一套严格的规则，并且和 C/Java 这类语言有本质区别。

## 作用域的 4 种类型（LEGB 规则）
Python 查找一个变量时，会按照固定的顺序在以下 4 个区域里找，这个顺序叫 LEGB：

- 1.L（Local）局部作用域：当前函数内部的变量。

- 2.E（Enclosing）嵌套作用域：外层函数的变量（只在函数嵌套时出现）。

- 3.G（Global）全局作用域：当前模块（.py 文件）最顶层定义的变量。

- 4.B（Built-in）内置作用域：Python 自带的名称，比如 `print`、`len`、`int`。

查找规则：从内到外，找到第一个就停下，找不到就报错 `NameError`。

***if、for、while 不创建新作用域（Python 独有）***

在 C/Java 里，花括号 {} 会创建新作用域。但在 Python 里，只有函数、类、模块会创建新作用域。if、for、while 内部的变量，外部依然可以访问。

```python
if True:
    x = 100
print(x)   # 输出：100（不会报错！if 没有独立作用域）

for i in range(3):
    pass
print(i)   # 输出：2（循环结束后 i 依然存在）
```
实用意义：你不需要像 C 语言一样担心循环变量的作用域泄漏问题——它本来就在外面。

***有关第四个Built-in（内置作用域）：***

它是什么？
是 Python 解释器启动时自动加载的一组名字。这些名字不需要你 import 任何模块，直接就能用。比如：
...
它存在一个叫 builtins 的模块里。你可以把它想象成 Python 给你配的“瑞士军刀”，开机就塞在你手里，你不用去商店买，也不用自己造。

你可以亲眼看到它里面有哪些东西：

```python
import builtins
print(dir(builtins))  # 会打印出一长串 Python 自带的函数和异常名字
```
（你在全局作用域里能用的那些东西，基本上都是从这里来的。）

🧠 最关键的区别：全局 vs 内置
很多人会把“全局”和“内置”搞混，因为它们看起来都“不用 import 就能用”。它们的区别只有一条：

全局（Global）：你自己在 .py 文件最顶层定义的变量。

内置（Built-in）：Python 系统提前帮你定义好的函数和类型。
...
# 这是内置函数（Python 自带的）
print(my_number)   # print 不是全局，也不是局部，它是内置的
```
⚠️ 为什么 LEGB 要把 Built-in 放在最后？
因为你可以覆盖它。

如果你在全局作用域里自己定义一个叫 print 的变量，Python 会优先用你的，而忽略内置的那个：

```python
def print(x):   # 你在全局定义了一个 print
    print("不会执行这行，因为会无限递归")
    return "我自己"

result = print("你好")   # 调用的是你定义的 print，不是内置的 print
print(result)            # 你定义的 print 返回了 "我自己"，这里输出：我自己
```
（不过，除非你在做极特殊的实验，否则绝对不要这么干——你会把 print 功能弄坏，导致后面没法调试。）

Built-in 放在最后，意味着它只是“保底方案”——只要你在局部、全局（或嵌套）自己定义了一个同名变量，Python 就不会去碰内置的那个。这是语言设计者故意留给你的灵活空间。
...
```
生命周期：函数被调用时创建，函数执行结束立即销毁。

外部无法访问：函数外面看不到里面的任何变量。

### 全局作用域

在函数外部、.py 文件最顶层定义的变量，就是全局变量。

```python
b = 20   # 全局变量

def func():
    print(b)   # 可以读取全局变量（只读，没问题）

func()        # 输出：20
print(b)      # 输出：20
```
作用范围：在当前文件的任何位置都可以访问（除非被同名的局部变量遮盖,被称为“遮盖”效应（Shadowing））。

生命周期：从定义开始，到程序结束才销毁。

### 函数内部修改全局变量（`global`）

如果在函数内部对变量进行“赋值”（=），Python 会默认把它当作局部变量。

- 场景 1：只读取，不修改（不需要任何声明）
...
read()  # 30
```
- 场景 2：试图修改，但没加 global（会出错或无效）

```python
c = 30
def modify():
    c = 40   # 你以为在改全局？错！这是在创建一个新的局部变量 c
    print(c) # 输出：40（局部）

modify()
print(c)    # 输出：30（全局没变！）
```
- 场景 3：先读取，再赋值（引发 UnboundLocalError）

```python
c = 30
def bug():
    print(c)   # 这里会报错！因为 Python 看到下面有 c = 40，已经把 c 标记为局部变量了
    c = 40

bug()
# 报错：UnboundLocalError: cannot access local variable 'c' where it is not associated with a value
```
原因：Python 在编译函数时，只要看到函数内部有 c = xxx 的赋值语句，就会把 c 标记为局部变量。即使打印语句在赋值之前，它也已经默认是局部变量了，而此时局部变量还没被赋值，所以报错。

- 场景 4：正确的修改方式（使用 global）

```python
c = 30
def correct():
    global c   # 声明：我要操作的是全局变量 c
    c = 40     # 现在改的就是全局的了

correct()
print(c)    # 输出：40（真的被改掉了）
```
global 的作用：告诉 Python，“不要在这个函数内创建新的局部变量，直接去全局作用域里找这个变量来操作”。

### `globals()` 和 `locals()`（内置函数）
...
def test():
    y = 20
    print(locals())   # {'y': 20}
print(globals())      # 包含 x 和一大堆 Python 内置变量
```
globals() 可以用于动态修改全局变量，但实际开发中极少需要这样做。

### 嵌套函数与 `nonlocal`（扩展了解）

如果在函数 A 内部再定义一个函数 B，B 想要修改 A 的变量，需要用 `nonlocal`。
```python
def outer():
    count = 0
    def inner():
        nonlocal count   # 声明：我要改的是 outer 里的 count
        count += 1
        print(count)
    inner()
    inner()
...
#      2
```
如果不加 `nonlocal`，`count += 1` 会在 `inner` 内部创建一个新的局部变量，`outer` 的 `count` 不会变化，且因为没赋值会报错。`nonlocal` 的使用场景较少，但你知道它的存在即可。
